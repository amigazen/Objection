...
again:	;
	...
	if ((op = evallex(opp->skip)) == OP_SUB && binop == 0)
	    ...
	    DBUG_RETURN (1);			/* Error in evallex	*/
#ifdef	DEBUG_EVAL
	...
	for (;;) {
		...
		if (op == OP_EOE)
		    DBUG_RETURN (valp[-1]);	/* Finished ok.		*/
...
again:  do {					/* Collect the token	*/
		...
		unget();
		DBUG_RETURN (OP_EOE);		/* End of expression	*/
	    }
	...
	if (t == INV) {				/* Total nonsense	*/
...
#endif
		DBUG_RETURN (DIG);		/* Return a value	*/
	    }
	    ...
	    case '<':
		if (c1 == c)
		    DBUG_RETURN ((c == '<') ? OP_ASL : OP_ASR);
		else if (c1 == '=')
		    DBUG_RETURN ((c == '<') ? OP_LE  : OP_GE);
		break;

	    case '|':
	    case '&':
		if (c1 == c)
		    DBUG_RETURN ((c == '|') ? OP_ORO : OP_ANA);
		break;
