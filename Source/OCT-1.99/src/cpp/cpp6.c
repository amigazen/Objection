 ...
 * 7-nov-90  bjw        Accepts C++ comment style ("//...").
 *                      cwarn(): bad options display "from commandline".
 */
...
#ifdef AZTEC_C
/*  At 3.6a, Aztec "C" doesn't have ANSI library, so force it! */
 ...
 *		expanded, or for #import remembering.
...
#ifdef amiga
		/*
		 * Duplicate the action of incmem, which grows the area
		 * to the new size, reallocating and copying if necessary.
		 * The disadvantage of this emulation is that the copy is
		 * always done because we have no realloc().
		 */
	      {
		 extern char *malloc ();
		 char *new = malloc ((unsigned) (1 + (tokenbsize *= 2)));
		 if (new == NULL) {
		    cfatal("Out of memory", NULLST);
		 } else {
		    memcpy (new, tokenbuf, ct);
		    free (tokenbuf);
		    tokenbuf = new;
		 }
	      }
...
#else
	DBUG_RETURN (FALSE);				/* Not supported	*/
#endif
}

int
 ...
 */
{
	...
	if ((dotflag = (c == '.')) != FALSE) {	/* . something?		*/
		...
		DBUG_VOID_RETURN;		/* All done for now	*/
	    }
	}					/* End of float test	*/
 ...
 *  Links only to global 'infile'.  If bufsize == 0, then saving name
 *  for #import directive.
 */
{
	...
	file = (FILEINFO *) getmem((int)(sizeof (FILEINFO) + bufsize + size));
 ...
 */
{
	...
	if (isrecurse)				/* Recursive definition	*/
	    DBUG_RETURN (NULL);			/* undefined just now	*/
 ...
 */
{
	...
	if (!delete) {
	    dp = (DEFBUF *) getmem((int) (sizeof (DEFBUF) + size));
	    dp->link = *prevp;
	    *prevp = dp;
	    dp->hash = nhash;
	    dp->repl = NULL;
 ...
 */
{
	...
	int                     cmt2;           /* Second char of comment */

	DBUG_ENTER ("get");
	popped = 0;
get_from_file:
	if ((file = infile) == NULL)
	    DBUG_RETURN (EOF_CHAR);
newline:
#if 0
	...
	if ((c = *file->bptr++ & 0xFF) == EOS) {
		...
		DBUG_RETURN (EOF_CHAR);		/* Return end of file	*/
...
test:		if (keepcomments && c != EOF_CHAR)
		...
		switch (c) {
		case EOF_CHAR:
	            instring = FALSE;	      /* Gotta end comment here */
		...
		case '\n':
		    if( cplusplus && cmt2 == '/' ) {
		    /* 
		     *  A C++ comment always continues to the end of the
		     *  line.  Since there can't be any text afterwards,
		     *  we can continue looking for a char.
		     */
		...
		case '*':
		     ...
		     */
		    DBUG_RETURN ((file->bptr[-1] = COM_SEP));
#else
		    DBUG_RETURN ((file->bptr[-1] = ' '));
#endif
		    /*NOTREACHED*/
		...
		unget();			/* Get it later		*/
		DBUG_RETURN ('\\');		/* Return the backslash	*/
	    }
	}
	else if (c == '\f' || c == VT)		/* Form Feed, Vertical	*/
	    c = ' ';				/* Tab are whitespace	*/
	DBUG_RETURN (c);			/* Just return the char	*/
}

unget()
/*
 ...
 */
{
	...
	DBUG_ENTER ("unget");
	if ((file = infile) == NULL)
	    DBUG_VOID_RETURN;		/* Unget after EOF		*/
 ...
 */
{
...
#ifdef m68000
	  fprintf(stderr, format, (long) arg);
#else
	...
	if( infile == NULL )	/* bjw */
	{
	    fprintf(stderr, " from commandline args\n" );
	} else
