...
description
	...
	.lm +16
	.p -16
	-+      	If set, accept C++ comment style.  This
	style begins with two forward slashes.  The rest of the
	line is treated as a comment.  (Compatible with the GNU cpp.)
	.p -16
 ...
 * are predefined.
 *
 * cplusplus    Nonzero means handle C++ comment syntax.  Should use
 *              additioanl C++ include directories (sigh).
 */
...
int             cplusplus = FALSE;      /* Accept C++ comment style     */
...
#if DEBUG
	fprintf(stderr,"using only i/o redirection\n" );
#endif
