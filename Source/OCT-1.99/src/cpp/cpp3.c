 ...
 * 15-Oct-90    bjw  Added #import; just like NeXT cpp
 * 7-nov-90     bjw     Clips C++ comments "// ..."
 */
...
#include	<signal.h>
/* extern int	abort();	(now in stdlib.h) -- For debugging   */
#endif


FILE_LOCAL	FILEINFO	*knownfiles = NULL;  /* Ignore list for #import */

/* ------------------------------------------------------------------- */

int
openfile(filename,include_flag)
char		*filename;
int         include_flag;	/* 1=#include, 0=#import */
/*
 * Open a file, add it to the linked list of open files.
 * This is called only from openinclude() in cpp2.c.
 */
{
	register FILE		*fp;
	extern FILE		*fopen ();
	...
	DBUG_ENTER ("openfile");
	if( !include_flag && seenfile(filename) ) {
	    /*  #import'ing an already included file.. */
		DBUG_RETURN( TRUE );	/* fake a read */
	}
...
/*
 *  Check if import'ed file already read.  Returns TRUE if filename
 *  is on our list.
 */
{
	...
	DBUG_ENTER ("seenfile");
#if SW_IMPORT
	for( file=knownfiles ; file != NULL ; file=file->parent )
	{
		if( streq(filename,file->filename) ) {
#if DEBUG
	if (debug)
	fprintf( stderr, "+Have seen %s+\n", filename );
#endif
			DBUG_RETURN (TRUE);
		}
	}
#if DEBUG
	if (debug)
	fprintf( stderr, "+have not seen %s+\n", filename );
#endif
#else
	printf( ">>>  #import  directive not configured!\n" );
#endif
	DBUG_RETURN (FALSE);	/* Not on list */
}
 ...
 *  The filename is remembered also.
 */
{
	register FILEINFO	*file;
	...
	file = getfile(0, filename);
	file->parent = knownfiles;  	/* Install name at head of list */
	knownfiles = file;
#endif

	/*  Maintain stack of "reading now" files: */
...
#define	MAXINCLUDE	(NINCLUDE - 1 - IS_INCLUDE)
	...
	*incend++ = "sys:include" ;	/* Some standard place */
	*incend++ = "sys:usr/include" ;
...
fprintf(stderr, "  -+\t\t\tAccept C++ style comments\n");
