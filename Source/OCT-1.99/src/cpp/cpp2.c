 ...
 * 21-Aug-89	bjw Amiga system always ignores text after #endif.
 *			Added #error directive.
 * 15-Oct-90    bjw  Added #import; just like NeXT cpp.  Skips #ident line.
 *  30-may-91  ramsey@rhrk.uni-kl.de (deon ramsey).  dir concat for Amiga.
 */
...
/*  Define TRUE  to dump text after #endif directive. */
#ifdef amiga
# define  SW_DUMP_AFTER  1
#endif

/*  The defaults: */
#ifndef SW_DUMP_AFTER
# define  SW_DUMP_AFTER  0
#endif
#ifndef SW_IMPORT
# define  SW_IMPORT  0
#endif
 ...
 *  _mk_hash() is primitive hasher, must use str[0],str[2].
 *  _strhash() accepts string (char *) to hash on.
 */
#define  _mk_hash(c0,c2)  ((c0)+((unsigned)(c2)<<3))
#define  _strhash(str)   _mk_hash((str)[0],(str)[2])


#define	L_assert	_mk_hash('a','s')
#define	L_define	_mk_hash('d','f')
#define	L_elif		_mk_hash('e','i')
#define	L_else		_mk_hash('e','s')
#define	L_endif		_mk_hash('e','d')
#define L_error         _mk_hash('e','r')
#define L_ident		_mk_hash('i','e')
#define	L_if		_mk_hash('i',EOS)
#define	L_ifdef		_mk_hash('i','d')
#define	L_ifndef	_mk_hash('i','n')
#define L_import        _mk_hash('i','p')	/* read file only once */
#define	L_include	_mk_hash('i','c')
#define	L_line		_mk_hash('l','n')
#define	L_nogood	_mk_hash(EOS,EOS)	/* To catch #i		*/
#define	L_pragma	_mk_hash('p','a')
#define L_undef		_mk_hash('u','d')
#if DEBUG
#define	L_debug		_mk_hash('d','b')	/* #debug		*/
#define	L_nodebug	_mk_hash('n','d')	/* #nodebug		*/
#endif
 ...
 */
{
	...
	hash = (tokenbuf[1] == EOS) ? L_nogood : _strhash(tokenbuf);
	switch (hash) {
	...
	case L_ident:	tp = "ident";    	break;	/* bjw - Jan 91 */
	...
	case L_undef:	tp = "undef";		break;
 	case L_error:   tp = "error";		break;	/* bjw - Aug 89 */
#if SW_IMPORT
 	case L_import:	tp = "import";      break;	/* bjw - Oct 90 */
#endif
#if DEBUG
	...
	{
	    if( hash== L_ident)
		fprintf(stderr," #ident in before any switch!!! compiling=%d\n", compiling);
	}
#endif
	/*
...
dump_line:	skipnl();			/* Ignore rest of line	*/
	...
	case L_import:
	case L_include:
 	    doinclude( hash == L_include );    /* dual action! (bjw - Oct 90 */
	    break;
...
nest_err:	cerror("#%s must be in an #if", tokenbuf);
	...
	case L_pragma:
	     ...
	     * #ident is passed onto compiler for possible object
	     *  code inclusion... (bjw - jan 91)
	     */
	    printf("#%s ", tp);
	    while ((c = get()) != '\n' && c != EOF_CHAR)
		cput(c);
 	...
 	case L_error:
	    ...
	    while ((c = get()) != '\n' && c != EOF_CHAR)
		 putc( c, stdout );
	    cerror( "#error encountered!", NULLST );
	    break;
	...
	if (hash != L_include && hash != L_import)
	{
	    ...
	    if (skipws() != '\n') {
#if SW_DUMP_AFTER
		/* If Amiga, always ignore text after #endif: (bjw) */
		if (hash != L_endif )
#endif
		cwarn("Unexpected text <%s> in #control line ignored", tokenbuf);
		skipnl();
	    }
#endif
	}
...

FILE_LOCAL
doinclude( include_flag )
int     include_flag;   	/* 1=include, 0=import */
/*
 ...
 *  (There are also "#import" variations.)
 *
 ...
 */
{
	...
	*workp = EOS;			/* Terminate filename		*/
	...
	else if (openinclude(def_filename, (delim == '"'),include_flag))
	    DBUG_VOID_RETURN;
#else
	if (openinclude(work, (delim == '"'),include_flag))
	    DBUG_VOID_RETURN;
#endif
	/*
...

FILE_LOCAL int
openinclude(filename, searchlocal,include_flag)
 ...
 */
{
...
#if HOST == SYS_UNIX
	if ((filename[0] == '/') && openfile(filename,include_flag)) {
	    DBUG_RETURN (TRUE);
	}
#endif
#if HOST == SYS_AMIGADOS
	if ((strchr (filename, ':') != NULL) && openfile (filename,include_flag)) {
	    DBUG_RETURN (TRUE);
	}
	...
	if (searchlocal) {
	    ...
	    if (openfile(tmpname,include_flag))
		DBUG_RETURN (TRUE);
	}
	/*
...
#if HOST == SYS_UNIX	/* Implied '/' */
		sprintf(tmpname, "%s/%s", *incptr, filename);
#else
#if HOST == SYS_AMIGADOS || HOST == SYS_MSDOS
		char *pos;
		if((pos=strchr(*incptr,':')) && (*(pos+1)==0) )
		    sprintf(tmpname, "%s%s", *incptr, filename);
		else
		    sprintf(tmpname, "%s/%s", *incptr, filename);
#else
